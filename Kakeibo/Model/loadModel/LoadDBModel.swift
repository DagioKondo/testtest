...
@objc protocol LoadOKDelegate {
    ...
    //変更③＜start＞-------------------------------------------
    @objc optional func loadGroupName_OK(groupName:String,groupImage:String,nextSettlementDay: Date)
    //変更③＜end＞---------------------------------------------
    ...
    func loadGroupName(groupID:String,activityIndicatorView:UIActivityIndicatorView){
        db.collection("groupManagement").document(groupID).addSnapshotListener { (snapShot, error) in
            ...
            if let data = snapShot?.data(){
                ...
                let groupImage = data["groupImage"] as! String
                //変更③＜start＞------------------------------------------
                let nextSettlementDay = data["nextSettlementDay"] as! Timestamp
                self.loadOKDelegate?.loadGroupName_OK?(groupName: groupName, groupImage: groupImage,nextSettlementDay: nextSettlementDay.dateValue())
                //変更③＜end＞--------------------------------------------
            }
        }
    }
